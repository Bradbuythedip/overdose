...
init_process();
init_buffer();
var import_buffer47 = __toESM(require_buffer());
  ...
  _connect(address, options) {
    ...
    this.socket.addEventListener("message", ({ data: message }) => {
      if (message instanceof ArrayBuffer)
        message = import_buffer38.Buffer.from(message).toString();
      try {
...
var toBuffer = (arr) => {
  if (import_buffer47.Buffer.isBuffer(arr)) {
    return arr;
  } else if (arr instanceof Uint8Array) {
    return import_buffer47.Buffer.from(arr.buffer, arr.byteOffset, arr.byteLength);
  } else {
    return import_buffer47.Buffer.from(arr);
  }
};
var Struct2 = class {
  ...
  encode() {
    return import_buffer47.Buffer.from((0, import_borsh.serialize)(SOLANA_SCHEMA, this));
  }
  static decode(data) {
...
var PublicKey = class _PublicKey2 extends Struct2 {
   ...
   */
  toBuffer() {
    const b = this._bn.toArrayLike(import_buffer47.Buffer);
    if (b.length === PUBLIC_KEY_LENGTH) {
      return b;
    }
    const zeroPad = import_buffer47.Buffer.alloc(32);
  ...
  static async createWithSeed(fromPublicKey, seed, programId) {
    const buffer = import_buffer47.Buffer.concat([fromPublicKey.toBuffer(), import_buffer47.Buffer.from(seed), programId.toBuffer()]);
  ...
  static createProgramAddressSync(seeds, programId) {
    let buffer = import_buffer47.Buffer.alloc(0);
    seeds.forEach(function(seed) {
      if (seed.length > MAX_SEED_LENGTH) {
        throw new TypeError(`Max seed length exceeded`);
      }
      buffer = import_buffer47.Buffer.concat([buffer, toBuffer(seed)]);
    });
    buffer = import_buffer47.Buffer.concat([buffer, programId.toBuffer(), import_buffer47.Buffer.from("ProgramDerivedAddress")]);
    const publicKeyBytes = sha256(buffer);
  ...
  static findProgramAddressSync(seeds, programId) {
    ...
    while (nonce != 0) {
      try {
        const seedsWithNonce = seeds.concat(import_buffer47.Buffer.from([nonce]));
...
var Account = class {
   ...
   */
  get secretKey() {
    return import_buffer47.Buffer.concat([this._secretKey, this._publicKey], 64);
  }
};
...
var rustString = (property = "string") => {
  ...
  rslShim.encode = (str, b, offset2) => {
    const data = {
      chars: import_buffer47.Buffer.from(str, "utf8")
    };
    return _encode(data, b, offset2);
  };
  rslShim.alloc = (str) => {
    return BufferLayout.u32().span + BufferLayout.u32().span + import_buffer47.Buffer.from(str, "utf8").length;
  };
  return rslShim;
};
...
var Message = class _Message {
  ...
  serialize() {
    ...
    const instructions = this.instructions.map((instruction) => {
      ...
      return {
        programIdIndex,
        keyIndicesCount: import_buffer47.Buffer.from(keyIndicesCount),
        keyIndices: accounts,
        dataLength: import_buffer47.Buffer.from(dataCount),
        data
      };
    });
    let instructionCount = [];
    encodeLength(instructionCount, instructions.length);
    let instructionBuffer = import_buffer47.Buffer.alloc(PACKET_DATA_SIZE);
    import_buffer47.Buffer.from(instructionCount).copy(instructionBuffer);
    ...
    const transaction = {
      numRequiredSignatures: import_buffer47.Buffer.from([this.header.numRequiredSignatures]),
      numReadonlySignedAccounts: import_buffer47.Buffer.from([this.header.numReadonlySignedAccounts]),
      numReadonlyUnsignedAccounts: import_buffer47.Buffer.from([this.header.numReadonlyUnsignedAccounts]),
      keyCount: import_buffer47.Buffer.from(keyCount),
      keys: this.accountKeys.map((key) => toBuffer(key.toBytes())),
      recentBlockhash: import_bs58.default.decode(this.recentBlockhash)
    };
    let signData = import_buffer47.Buffer.alloc(2048);
  ...
  static from(buffer) {
    ...
    for (let i = 0; i < accountCount; i++) {
      ...
      accountKeys.push(new PublicKey(import_buffer47.Buffer.from(account)));
    }
    ...
    for (let i = 0; i < instructionCount; i++) {
      ...
      const data = import_bs58.default.encode(import_buffer47.Buffer.from(dataSlice));
      instructions.push({
        programIdIndex,
        accounts,
        data
    ...
    const messageArgs = {
      ...
      recentBlockhash: import_bs58.default.encode(import_buffer47.Buffer.from(recentBlockhash)),
      accountKeys,
      instructions
    };
...
var DEFAULT_SIGNATURE = import_buffer47.Buffer.alloc(SIGNATURE_LENGTH_IN_BYTES).fill(0);
var TransactionInstruction = class {
  constructor(opts) {
    this.keys = void 0;
    this.programId = void 0;
    this.data = import_buffer47.Buffer.alloc(0);
...
var Transaction = class _Transaction {
  ...
  _addSignature(pubkey, signature2) {
    ...
    this.signatures[index].signature = import_buffer47.Buffer.from(signature2);
  }
  /**
...
Missing signature for public key${sigErrors.missing.length === 1 ? "" : "(s)"} [\`${sigErrors.missing.map((p) => p.toBase58()).join("`, `")}\`].`;
  ...
  _serialize(signData) {
    ...
    const wireTransaction = import_buffer47.Buffer.alloc(transactionLength);
    assert2(signatures.length < 256);
    import_buffer47.Buffer.from(signatureCount).copy(wireTransaction, 0);
    signatures.forEach(({
      signature: signature2
    }, index) => {
      if (signature2 !== null) {
        assert2(signature2.length === 64, `signature has invalid length`);
        import_buffer47.Buffer.from(signature2).copy(wireTransaction, signatureCount.length + index * 64);
      }
    });
  ...
  static from(buffer) {
    ...
    for (let i = 0; i < signatureCount; i++) {
      ...
      signatures.push(import_bs58.default.encode(import_buffer47.Buffer.from(signature2)));
    }
...
function encodeData(type2, fields) {
  ...
  const data = import_buffer47.Buffer.alloc(allocLength);
...
var bigInt = (length) => (property) => {
  ...
  bigIntLayout.decode = (buffer, offset2) => {
    ...
    return (0, import_bigint_buffer.toBigIntLE)(import_buffer47.Buffer.from(src));
  };
...
var Loader = class _Loader {
  ...
  static async load(connection, payer, program, programId, data) {
    ...
    while (array2.length > 0) {
      ...
      const data2 = import_buffer47.Buffer.alloc(chunkSize + 16);
      dataLayout.encode({
        instruction: 0,
        // Load instruction
        offset: offset2,
    ...
    await Promise.all(transactions);
    {
      ...
      const data2 = import_buffer47.Buffer.alloc(dataLayout2.span);
      dataLayout2.encode({
        instruction: 1
...
var BufferFromRawAccountData = coerce(instance(import_buffer47.Buffer), RawAccountDataResult, (value) => import_buffer47.Buffer.from(value[0], "base64"));
...
var ParsedOrRawAccountData = coerce(union([instance(import_buffer47.Buffer), ParsedAccountDataResult]), union([RawAccountDataResult, ParsedAccountDataResult]), (value) => {
  if (Array.isArray(value)) {
...
var Connection = class {
  ...
  async simulateTransaction(transactionOrMessage, configOrSigners, includeAccounts) {
    if ("message" in transactionOrMessage) {
      ...
      const encodedTransaction2 = import_buffer47.Buffer.from(wireTransaction2).toString("base64");
...
var Ed25519Program = class _Ed25519Program {
  ...
  static createInstructionWithPublicKey(params) {
    ...
    const instructionData = import_buffer47.Buffer.alloc(messageDataOffset + message.length);
...
var Secp256k1Program = class _Secp256k1Program {
  ...
  static publicKeyToEthAddress(publicKey2) {
    ...
    try {
      return import_buffer47.Buffer.from(keccak_256(toBuffer(publicKey2))).slice(-ETHEREUM_ADDRESS_BYTES);
    } catch (error) {
  ...
  static createInstructionWithEthAddress(params) {
    ...
    if (typeof rawAddress === "string") {
      if (rawAddress.startsWith("0x")) {
        ethAddress = import_buffer47.Buffer.from(rawAddress.substr(2), "hex");
      } else {
        ethAddress = import_buffer47.Buffer.from(rawAddress, "hex");
      }
    } else {
      ethAddress = rawAddress;
    }
    ...
    const numSignatures = 1;
    const instructionData = import_buffer47.Buffer.alloc(SECP256K1_INSTRUCTION_LAYOUT.span + message.length);
  ...
  static createInstructionWithPrivateKey(params) {
    ...
    try {
      ...
      const messageHash = import_buffer47.Buffer.from(keccak_256(toBuffer(message)));
  ...
  static fromConfigData(buffer) {
    ...
    if (configKeys[0].publicKey.equals(VALIDATOR_INFO_KEY)) {
      if (configKeys[1].isSigner) {
        const rawInfo = rustString().decode(import_buffer47.Buffer.from(byteArray));
...
@noble/curves/esm/secp256k1.js:
  (*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) *)
*/
//# sourceMappingURL=chunk-AHAN2R6N.js.map
